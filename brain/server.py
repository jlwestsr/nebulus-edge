...
@app.get("/")
def health_check():
    return {"status": "running", "current_model": current_model_name, "available_models": list(MODELS.keys())}
...
def chat_completions(request: ChatCompletionRequest):
    ...
    if requested_model != current_model_name:
        try:
             ...
             raise HTTPException(status_code=500, detail=f"Failed to load model {requested_model}: {str(e)}")
